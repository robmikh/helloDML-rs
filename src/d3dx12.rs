use windows::Win32::Graphics::{
    Direct3D12::{
        ...
        D3D12_HEAP_PROPERTIES, D3D12_HEAP_TYPE, D3D12_MEMCPY_DEST, D3D12_MEMORY_POOL_UNKNOWN,
        ...
        D3D12_RESOURCE_DIMENSION_BUFFER, D3D12_RESOURCE_FLAGS, D3D12_RESOURCE_STATES,
...
pub fn update_subresources(
    ...
    for i in 0..num_subresources {
        ...
        let dest_data = D3D12_MEMCPY_DEST {
            ...
            SlicePitch: layouts[i as usize].Footprint.RowPitch as usize
                * num_rows[i as usize] as usize,
        };
        memcpy_subresource(
            &dest_data,
            &src_data[i as usize],
            row_size_in_bytes[i as usize] as usize,
            num_rows[i as usize],
    ...
    if destination_desc.Dimension == D3D12_RESOURCE_DIMENSION_BUFFER {
        ...
        for i in 0..num_subresources {
            let dest_location =
                D3DX12TextureCopyLocation::new(destination_resource, i + first_subresource);
            let src_location =
                D3DX12TextureCopyLocation::new_from_footprint(intermediate, &layouts[i as usize]);
            unsafe {
...
pub fn update_subresource_heap(
    ...
    let mem_to_alloc = (std::mem::size_of::<D3D12_PLACED_SUBRESOURCE_FOOTPRINT>()
        + std::mem::size_of::<u32>()
        + std::mem::size_of::<u64>()) as u64
        * num_subresources as u64;
    ...
    let row_size_in_bytes = unsafe {
        std::slice::from_raw_parts_mut(
            layouts.as_mut_ptr().add(num_subresources as usize) as *mut u64,
    ...
    let num_rows = unsafe {
        std::slice::from_raw_parts_mut(
            row_size_in_bytes
                .as_mut_ptr()
                .add(num_subresources as usize) as *mut u32,
            num_subresources as usize,
        )
    };
    ...
    unsafe {
        d3d12_device.GetCopyableFootprints(
            &desc,
            first_subresource,
            num_subresources,
            intermediate_offset,
...
pub fn memcpy_subresource(
    ...
    for z in 0..num_slices {
        ...
        let dest_slice =
            unsafe { std::slice::from_raw_parts_mut(dest_ptr, row_size_in_bytes as usize) };
